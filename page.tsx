...
// ── Price chart (canvas-free SVG line) ────────────────────────────────────
function PriceChart({ ticker }) {
  ...
  const w = 600, h = 160, pad = { top: 10, right: 10, bottom: 24, left: 40 };
  const cw = w - pad.left - pad.right, ch = h - pad.top - pad.bottom;
  const min = Math.min(...prices) - 0.5, max = Math.max(...prices) + 0.5;
  const x = (i) => pad.left + (i / (prices.length - 1)) * cw;
  const y = (v) => pad.top + ch - ((v - min) / (max - min)) * ch;
  const linePts = prices.map((v, i) => `${x(i)},${y(v)}`).join(" L ");
  const fillPts = `M ${x(0)},${y(prices[0])} L ${linePts.slice(linePts.indexOf(" ") + 1)} L ${x(prices.length - 1)},${h - pad.bottom} L ${x(0)},${h - pad.bottom} Z`;
  const gridYs = [min, (min + max) / 2, max].map(v => ({ v, y: y(v) }));

  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: "auto" }}>
      {gridYs.map(({ v, y: gy }) => (
        <g key={v}>
          <line x1={pad.left} x2={w - pad.right} y1={gy} y2={gy} stroke="rgba(128,128,128,0.12)" strokeWidth="1" />
          <text x={pad.left - 4} y={gy + 4} textAnchor="end" fontSize="9" fill="#888">${v.toFixed(0)}</text>
        </g>
      ))}
      <defs>
      ...
      <path d={fillPts} fill="url(#fillGrad)" />
      <polyline points={prices.map((v, i) => `${x(i)},${y(v)}`).join(" ")} fill="none" stroke="#22c55e" strokeWidth="1.8" strokeLinejoin="round" />
      {labels.filter((_, i) => i % 2 === 0).map((l, idx) => {
        const i = idx * 2;
        return <text key={l} x={x(i)} y={h - 4} textAnchor="middle" fontSize="9" fill="#888">{l}</text>;
      })}
    </svg>
  );
}
...
// ── Fake sparkline data per ticker ────────────────────────────────────────
const sparkData = {
...
export default function Dashboard() {
  ...
  const [aiMessage, setAiMessage] = useState(
    "RSI at 62 — not overbought. MACD showing a bullish crossover. Recent news sentiment is net positive. Recommendation: hold current position and watch for a dip to the $175 support level as a potential entry point."
  );
  ...
  const handleTickerChange = (t) => {
    ...
    const msgs = {
      ...
      NVDA: "Strong institutional buying detected. Earnings expectations are being revised upward. RSI 58 — room to run. Consider buying on any minor pullback.",
      ...
      <div style={{ borderBottom: "0.5px solid #1e1e28", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 28, height: 28, background: "#22c55e", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><polyline points="1,10 4,6 7,8 10,3 13,5" stroke="#000" strokeWidth="1.8" strokeLinejoin="round" strokeLinecap="round"/></svg>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#1e1e28", border: "0.5px solid #2a2a35", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#888" }}>U</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 10 }} className="fade-up">
          {[
            ...
            <div key={m.label} className="card">
              ...
              <div style={{ fontSize: 22, fontWeight: 500, color: m.up === true ? "#22c55e" : m.up === false ? "#ef4444" : "#e8e8ec", fontFamily: "'Syne', sans-serif" }}>{m.value}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div className="card fade-up" style={{ animationDelay: "0.05s" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {TICKERS.map(t => (
                    <button key={t} className={`ticker-btn${activeTicker === t ? " active" : ""}`} onClick={() => handleTickerChange(t)}>{t}</button>
                  ))}
                </div>
              </div>
            ...
            <div className="card fade-up" style={{ animationDelay: "0.1s" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 8 }}>
                {INDICATORS.map(ind => (
                  <div key={ind.name} className="row-hover" style={{ border: "0.5px solid #1e1e28", borderRadius: 8, padding: "10px 12px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div className="card fade-up" style={{ animationDelay: "0.08s" }}>
              ...
              <p style={{ fontSize: 12, color: "#aaa", lineHeight: 1.7, marginBottom: 14 }}>{aiMessage}</p>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderTop: "0.5px solid #1e1e28" }}>
                ...
                <div
                  ...
                  style={{ width: 36, height: 20, background: autoTrade ? "#22c55e" : "#2a2a35", borderRadius: 10, position: "relative", cursor: "pointer", transition: "background 0.2s" }}
                >
              ...
              {autoTrade && (
                <div style={{ fontSize: 11, color: "#f59e0b", background: "#f59e0b18", borderRadius: 6, padding: "6px 10px", marginTop: 6 }}>
                  ⚠ Auto-trade on. Paper trading only — connect broker to go live.
                </div>
              )}
            </div>

            {/* Trade signals */}
            <div className="card fade-up" style={{ animationDelay: "0.12s", flex: 1 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {SIGNALS.map(s => (
                  <div key={s.ticker} className="row-hover" style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={() => handleTickerChange(s.ticker)}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div className="card fade-up" style={{ animationDelay: "0.15s" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {SIGNALS.map(s => (
                <div key={s.ticker} className="row-hover" style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => handleTickerChange(s.ticker)}>
